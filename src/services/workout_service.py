...
from src.models.models import User, Workout, WorkoutType
from src.schemas.workout import WorkoutCreate, WorkoutStats, WorkoutUpdate
...
class WorkoutService:
    ...
    async def create_workout(
        ...
        logger.info("Создание новой тренировки для пользователя: ID %d", user_id)

        user_result = await db.execute(select(User).where(User.id == user_id))
        user = user_result.scalar_one_or_none()

        if not user:
            logger.error("Пользователь не найден: ID %d", user_id)
        ...
        if (
            workout_data.workout_type == WorkoutType.SWIMMING
            and workout_data.pool_length_m
            and workout_data.pool_laps
        ):
        ...
        logger.info("Тренировка создана успешно: ID %d", workout.id)
        return workout

    @staticmethod
    async def get_workout_by_id(db: AsyncSession, workout_id: int, user_id: int) -> Workout | None:
    ...
    async def update_workout(
        ...
        logger.info("Обновление тренировки: ID %d", workout_id)
        ...
        logger.info("Тренировка обновлена успешно: ID %d", workout_id)
        return workout

    @staticmethod
    async def delete_workout(db: AsyncSession, workout_id: int, user_id: int) -> bool:
        logger.info("Удаление тренировки: ID %d", workout_id)
        ...
        logger.info("Тренировка удалена успешно: ID %d", workout_id)
        return True
    ...
    async def get_workout_statistics(
        ...
        logger.info("Получение статистики тренировок для пользователя: ID %d", user_id)
