...
def test_InfoMessage():
    ...
    for p in ["training_type", "duration", "distance", "speed", "calories"]:
        assert p in info_message_signature_list, (
            f"У метода `__init__` класса `InfoMessage` должен быть параметр {p}."
        )
...
def test_InfoMessage_get_message(input_data, expected):
    ...
    assert isinstance(result, str), (
        "Метод `get_message` в классе `InfoMessage`должен возвращать значение типа `str`"
    )
...
def test_Training():
    ...
    for name, value in {
        ...
        assert hasattr(homework.Training, name), (
            f"У класса `Training` должна быть константа `{name}`{value.description}."
        )
        assert getattr(homework.Training, name) == value.value, (
            f"Значение константы `{name}` в классе `Training` должно быть равно `{value.value}`."
        )
    MIN_IN_H = 60
    ...
    for param in ["action", "duration", "weight"]:
        assert param in training_signature_list, (
            f"У метода `__init__` класса `Training` должен быть  параметр {param}."
        )
...
def test_Swimming():
    ...
    for value_info in {
        "CALORIES_MEAN_SPEED_SHIFT": ValueInfo(1.1, (" для смещения значения средней скорости")),
    ...
    for param in ["action", "duration", "weight", "length_pool", "count_pool"]:
        assert param in swimming_signature_list, (
            f"У метода `__init__` класса `Swimming`  должен быть параметр {param}."
        )
    assert "LEN_STEP" in list(swimming.__dict__), "Задайте константу `LEN_STEP` в классе `Swimming`"
...
def test_SportsWalking():
    ...
    assert inspect.isclass(homework.SportsWalking), "`SportsWalking` должен быть классом."
    ...
    for value_info in {
        "CALORIES_WEIGHT_MULTIPLIER": ValueInfo(0.035, " для множителя веса спортсмена"),
        "CALORIES_SPEED_HEIGHT_MULTIPLIER": ValueInfo(
            0.029, (" для множителя частного квадрата средней скорости и роста спортсмена")
        ),
    ...
    for param in ["action", "duration", "weight", "height"]:
        assert param in sports_walking_signature_list, (
            f"У метода `__init__` класса `SportsWalking` должен быть параметр {param}."
        )
...
def test_Running():
    ...
    for value_info in {
        "CALORIES_MEAN_SPEED_MULTIPLIER": ValueInfo(18, " для множителя средней скорости"),
...
def test_main():
    assert hasattr(homework, "main"), "Создайте главную функцию программы с именем `main`."
...
def test_main_output(input_data, expected):
    ...
    assert get_message_output == expected, "Метод `main` должен печатать результат в консоль.\n"
